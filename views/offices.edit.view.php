...
/** @var array $office */ /** @var array $rollen */ ?>
<div class="max-w-xl mx-auto p-6 space-y-6">
  <div class="flex justify-between items-baseline">
    <h1 class="text-xl font-semibold"><?= htmlspecialchars($title) ?></h1>
    <a href="/grundlagen/schulverwaltung/offices.php"
      class="text-sm text-gray-600 dark:text-gray-400 hover:underline">Zurück</a>
  </div>

  <form method="post" action="/grundlagen/schulverwaltung/actions/offices/edit.php"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input id="vorname" name="vorname" type="text" required
          value="<?= htmlspecialchars($office['vorname'] ?? '') ?>"
      ...
      <div>
        ...
        <input id="nachname" name="nachname" type="text" required
          value="<?= htmlspecialchars($office['nachname'] ?? '') ?>"
          class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900">
      </div>
    </div>
    ...
    <div id="verwaltungsrolle-select" class="relative">
      ...
      <div data-panel
        ...
        <ul class="max-h-60 overflow-auto divide-y divide-gray-100 dark:divide-gray-700">
          ...
          <?php
            $rid   = (int)$r['id'];
            $label = htmlspecialchars($r['label'] ?? ('Rolle #' . $rid));
            $sel   = (int)$office['verwaltungs_rolle_id'] === $rid;
            ?>
          <li>
      ...
      <div data-chip class="mt-2 text-sm">
        <?php foreach ($rollen as $r): ?>
        <?php $rid = (int)$r['id'];
          if ($rid === (int)$office['verwaltungs_rolle_id']): ?>
        <span data-chip-item
      ...
      <div data-hidden>
        <input type="hidden" name="verwaltungs_rolle_id" value="<?= (int)$office['verwaltungs_rolle_id'] ?>">
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function select(id, label) {
    ...
    options.forEach(btn => {
      const is = btn.getAttribute('value') === id;
  ...
  if (current) {
    const act = options.find(o => o.getAttribute('value') === current);
    if (act) labelEl.textContent = act.querySelector('span').textContent.trim();
  }
